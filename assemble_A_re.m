function out_A_re = assemble_A_re(freq, epsilon)
...
if std_freq/mean_freq<1  %(error in frequency difference <1% make sure that the terms are evenly distributed)
    for iter_freq_n = 1: numel(freq)
        freq_n = freq(iter_freq_n);
            freq_m = freq(1);
            C(iter_freq_n, 1) = g_i(freq_n, freq_m, epsilon);
    end       
        for iter_freq_m = 1: numel(freq)

            freq_n = freq(1);
            freq_m = freq(iter_freq_m);
            R(1, iter_freq_m) = g_i(freq_n, freq_m, epsilon);
    ...
    for iter_freq_n = 1: numel(freq)
        ...
        for iter_freq_m = 1: numel(freq)
            ...
            freq_n = freq(iter_freq_n);
            freq_m = freq(iter_freq_m);
            out_A_re_temp(iter_freq_n, iter_freq_m) = g_i(freq_n, freq_m, epsilon);
